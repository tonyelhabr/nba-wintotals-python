        ...
        for row in sched_data:
            ...
            if sched_team == scores_df2.loc[index, 'team_schedule'] and\
                ...
                try:
                    ...
                    if home_bool:
                        if win_bool:
                            scores_df2.loc[index, 'home_score'] = both_scores[0]
                            scores_df2.loc[index, 'away_score'] = both_scores[1]
                        else:
                            scores_df2.loc[index, 'home_score'] = both_scores[1]
                            scores_df2.loc[index, 'away_score'] = both_scores[0]
                    else:
                        if win_bool:
                            scores_df2.loc[index, 'home_score'] = both_scores[1]
                            scores_df2.loc[index, 'away_score'] = both_scores[0]
...
def save_scores_df2_to_csv(scores_f, scores_df2, year, timestamp):
    ...
    scores_f_removed = 'csvs\\nba-season-scores-{0}-test-backup.csv'.format(
        year)
    ...
    if not os.path.isfile(scores_f):
        ...
        with open(scores_f, 'rb') as f:
            ...
            for row in reader:
                ...
                if row[9] == 'N/A' and row[2] < date.today():
            ...
            while answer != 'y' or answer != 'n':
                ...
                if answer == 'y':
                    with open(scores_f, 'rb') as f_in, open(scores_f_temp, 'wb') as f_out:
                        ...
                        index = 0
                        for row in reader:
                            if row[0] == 'N/A' and row[2] < date.today:
                    ...
                    scores_df2.to_csv(
                        scores_f, mode='a', header=False, index=False)
...
def save_scores_df2_to_csv_v2(scores_f, year, timestamp):
    scores_f_add = 'csvs\\nba-season-scores-{0}-test2-add.csv'.format(year)
    scores_f_temp = 'csvs\\nba-season-scores-{0}-test2-temp.csv'.format(year)
    scores_f_backup = 'csvs\\nba-season-scores-{0}-test2-backup.csv'.format(
        year)

    # scores_f_add_text = open(scores_f_add, 'rb')
    # scores_df2.to_csv(scores_f_add)
    scores_df2 = pd.read_csv(scores_f_add, na_filter=False, parse_dates=['game_date'])
    # scores_f_add_text.close()
    # print "Created new file with game scores to add '{0}' at
    # {1}.".format(scores_f_add, timestamp)
    ...
    with open(scores_f, 'rb') as f:
        ...
        for row in reader:
            ...
            if not row[0] == 'game_id':
                ...
                game_date = date(date_raw.year, date_raw.month, date_raw.day)
                ...
                with open(scores_f, 'rb') as f_in, open(scores_f_temp, 'wb') as f_out:
                    ...
                    for row in reader:
                        ...
                        if not row[0] == 'game_id':
                            ...
                            if row[9] == 'N/A' and game_date < date.today() and index < len(scores_df2['game_date']):
                                writer.writerow(scores_df2.iloc[index])
                                index += 1
                            else:
                                writer.writerow(row)
...
def main2():
    ...
    scores_f = 'csvs\\nba-season-scores-{0}-test2.csv'.format(year)
    ...
    save_scores_df2_to_csv_v2(scores_f, year, timestamp)
